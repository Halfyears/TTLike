            ...
            {tier && (
              <div className="rounded-xl bg-gray-50 px-4 py-3 space-y-1.5">
                <div className="flex justify-between text-xs">
                  <span className="text-gray-500">Video analyses</span>
                  <span className="font-semibold text-gray-900">{tier.video_analysis_used} / {tier.video_analysis_limit}</span>
                </div>
                <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-pink-500 to-violet-500 transition-all"
                    style={{ width: `${tier.video_analysis_limit > 0 ? Math.min(100, Math.round(tier.video_analysis_used / tier.video_analysis_limit * 100)) : 0}%` }}
                  />
                </div>
              </div>
            )}
