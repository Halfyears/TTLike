...
const SECTIONS: Section[] = [
  {
    ...
    items: [
      ...
      { id: 'e2', text: 'Background is clean and intentional — no laundry, clutter, or distracting movement', tip: 'A plain wall, shelf with books, or branded backdrop all work well' },
      ...
      { id: 'e5', text: 'Temperature is comfortable — you won\'t need to adjust clothing mid-take', tip: 'Comfort shows on camera. Cold or sweating = distracted delivery' },
      { id: 'e6', text: 'Notification banners are off — Do Not Disturb mode enabled on all devices', tip: 'A notification banner on screen is an instant retake trigger' },
    ],
  },
  {
    id:       'equipment',
    ...
    items: [
      { id: 'q1', text: 'Camera / phone battery is above 50% and there\'s enough storage space', tip: 'Nothing worse than stopping mid-take for a low battery warning' },
      { id: 'q2', text: 'Camera lens is clean — wipe with a microfibre cloth', tip: 'Fingerprints cause blurry, hazy footage even in good lighting' },
      ...
      { id: 'q5', text: 'Product / props are clean, charged, and within arm\'s reach', tip: 'Set out everything you\'ll need before you start — don\'t improvise mid-shoot' },
      { id: 'q6', text: 'B-roll shots planned — close-ups, detail shots, hands-on demo shots identified', tip: 'B-roll covers cuts and makes edits smooth. Plan at least 3 cutaway angles' },
      { id: 'q7', text: 'Script or hook line is visible if needed — cue card, teleprompter, or phone taped to camera', tip: 'Looking slightly off-lens for a cue card reads better than looking down at hands' },
      { id: 'q8', text: 'Frame and crop is checked — face positioned in upper third, product visible', tip: 'TikTok is 9:16. Leave space at top/bottom for captions and UI overlays' },
    ],
  },
  {
    id:       'mental',
    ...
    items: [
      { id: 'm1', text: 'Hook line is memorised — you can say it naturally, not read it', tip: 'The first 2 seconds are everything. Practice the hook 5 times before recording' },
      { id: 'm2', text: 'Key message is clear — you know the ONE thing viewers must take away', tip: 'If you can\'t say it in one sentence, you\'re not ready to film yet' },
      { id: 'm3', text: 'Energy level is high — you\'ve moved, stretched, or had water', tip: 'TikTok rewards high energy. Do 10 jumping jacks before recording if needed' },
      { id: 'm4', text: 'Delivery speed is planned — slightly faster than normal conversation', tip: 'Slow delivery loses viewers. Aim for energetic but clear, not rushed' },
    ...
    <div className={`rounded-2xl border transition-colors ${allDone ? 'border-emerald-200 bg-emerald-50/40' : `${section.border} bg-white`}`}>
      ...
      >
        <div className={`h-9 w-9 rounded-xl ${allDone ? 'bg-emerald-100' : section.bg} flex items-center justify-center shrink-0`}>
          <section.icon className={`w-4.5 h-4.5 ${allDone ? 'text-emerald-600' : section.color}`} />
        </div>
        ...
        <ProgressRing
          ...
          color={section.color === 'text-amber-500' ? '#f59e0b' : section.color === 'text-blue-500' ? '#3b82f6' : '#8b5cf6'}
        />
    ...
    <div className="space-y-5 pb-4">
      ...
      <div className={`rounded-2xl px-5 py-4 border transition-colors ${allDone ? 'bg-emerald-50 border-emerald-200' : 'bg-gray-50 border-gray-100'}`}>
        ...
        {allDone && (
          <p className="text-xs text-emerald-600 mt-2 font-medium">
            All {totalItems} items checked. Hit record — you&apos;ve got this! 🚀
          </p>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-pink-100 bg-gradient-to-br from-pink-50 to-violet-50 p-5">
        ...
        <ul className="space-y-2">
          {[
            { icon: Wind,    text: 'Record your first take within 60 seconds of being ready — overthinking kills energy' },
            { icon: Monitor, text: 'Watch back your take with sound OFF first — check body language and energy before audio' },
      ...
      {/* CTA — go generate script first if they haven't */}
